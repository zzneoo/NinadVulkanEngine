...
VkResult DescriptorSetLayouts::createDescriptorSetLayout_FrameData(void)
{
    ...
    vkDescriptorSetLayoutBinding_Array[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_TASK_BIT_EXT | VK_SHADER_STAGE_MESH_BIT_EXT | VK_SHADER_STAGE_COMPUTE_BIT; // stage flags for the descriptor set layout binding
...
VkResult DescriptorSetLayouts::createDescriptorSetLayout_VolumetricClouds(void)
{
    ...
    // variable declarations
    VkResult vkResult = VK_SUCCESS;
    VkDescriptorSetLayoutBinding vkDescriptorSetLayoutBinding_Array[5]{};
    vkDescriptorSetLayoutBinding_Array[0].binding = 0; // binding index
    ...
    vkDescriptorSetLayoutBinding_Array[4].binding = 4; // binding index
    vkDescriptorSetLayoutBinding_Array[4].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; // descriptor type
    vkDescriptorSetLayoutBinding_Array[4].descriptorCount = 1; // number of descriptors
    vkDescriptorSetLayoutBinding_Array[4].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT; // stage flags for the descriptor set layout binding
    vkDescriptorSetLayoutBinding_Array[4].pImmutableSamplers = NULL; // no immutable samplers
    // code
    VkDescriptorSetLayoutCreateInfo vkDescriptorSetLayoutCreateInfo{};
    vkDescriptorSetLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    vkDescriptorSetLayoutCreateInfo.pNext = NULL;
    vkDescriptorSetLayoutCreateInfo.flags = 0;
    vkDescriptorSetLayoutCreateInfo.bindingCount = 5;
    vkDescriptorSetLayoutCreateInfo.pBindings = vkDescriptorSetLayoutBinding_Array; // pointer to the descriptor set layout binding array
...
VkResult DescriptorSetLayouts::createDescriptorSetLayout_AccumOpticalDepth(void)
{
    ...
    vkDescriptorSetLayoutCreateInfo.bindingCount = 3;
    vkDescriptorSetLayoutCreateInfo.pBindings = vkDescriptorSetLayoutBinding_Array; // pointer to the descriptor set layout binding array
    vkResult = vkCreateDescriptorSetLayout(gVulkanContext.vkDevice, &vkDescriptorSetLayoutCreateInfo, NULL, &vkDescriptorSetLayout_AccumOpticalDepth);
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFILE, "createDescriptorSetLayout_AccumOpticalDepth() -> vkCreateDescriptorSetLayout() :  failed: %d.\n", vkResult);
        return(vkResult);
    }
...
VkResult DescriptorSetLayouts::createDescriptorSetLayouts(void)
{
    ...
    vkResult = createDescriptorSetLayout_AccumOpticalDepth();
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFILE, " createDescriptorSetLayout_AccumOpticalDepth() failed (%d).\n", vkResult);
        return vkResult;
    }
    vkDescriptorSetLayoutList.push_back(vkDescriptorSetLayout_AccumOpticalDepth);
